...
public class LsetCmd implements Command {
	...
	private static final TclObject flat(Interp interp, TclObject list,
		...
		for (int i = 0;; ++i) {
			...
			try {
				...
				/*
				 * If we can't get index of indices table, we break the loop and
				 * then prepare for return from method.
				 */
				break;
			}
		...
		if (result == TCL.OK) {
			try {
				 ...
				 * Falls through!
				 * 
				 * Probably the TclList.setElement() threw an exception. It's no
				 * sense to continue so we break this block and continue
				 * execution cleaning up the retValue reference.
				 */
			}
		}
	 ...
	 *        --------------
	 */
	private static final TclObject list(Interp interp, TclObject list,
			TclObject indexArg, TclObject value) throws TclException {
