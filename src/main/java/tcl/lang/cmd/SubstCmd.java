...
public class SubstCmd implements Command {
	static final private String validCmds[] = { "-nobackslashes",
	...
	public void cmdProc(Interp interp, TclObject argv[]) throws TclException {
		...
		for (currentObjIndex = 1; currentObjIndex < objc; currentObjIndex++) {
			...
			int opt = TclIndex.get(interp, argv[currentObjIndex], validCmds,
					"switch", 0);
			switch (opt) {
			...
			case OPT_NOVARS:
				...
				break;
			default:
				throw new TclException(interp, "SubstCmd.cmdProc: bad option "
						+ opt + " index to cmds");
			}
		}
