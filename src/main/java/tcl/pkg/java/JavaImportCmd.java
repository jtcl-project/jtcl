...
public class JavaImportCmd implements Command {
			...
			throws TclException // A standard Tcl exception.
	{
		...
		if (objv.length == 1) {
			...
			for (Iterator iter = classTable.entrySet().iterator(); iter
					.hasNext();) {
				Map.Entry entry = (Map.Entry) iter.next();
				String key = (String) entry.getKey();
				String value = (String) entry.getValue();
				TclList.append(interp, import_list, TclString
						.newInstance(value));
				if (debug) {
		...
		if (startIdx >= objv.length) {
			if (forget) {
				...
				for (Iterator iter = packageTable.entrySet().iterator(); iter
						.hasNext();) {
					Map.Entry entry = (Map.Entry) iter.next();
					...
					if (elem.equals(pkg)) {
						...
						if (debug) {
							System.out.println("matched package to remove "
									+ pkg);
							System.out.println("will remove each of {" + alist
									+ "}");
						}

						for (ListIterator iter2 = alist.listIterator(); iter2
								.hasNext();) {
							// Remove imported class from the classTable
							elem2 = (String) iter2.next();
							if (debug) {
								System.out.println("remove " + elem2
										+ " entry from classTable");
							}
							if (classTable.remove(elem2) == null) {
								throw new TclRuntimeError("key " + elem2
										+ " not in classTable");
							}
						}
						...
						if (packageTable.remove(elem) == null) {
							throw new TclRuntimeError("key " + elem
									+ " not in packageTable");
						}
					}
				}
				...
				if (!found) {
					...
					throw new TclException(interp, "cannot forget package \""
							+ pkg + "\", no classes were imported from it");
				}
					...
					if (elem.equals(pkg)) {
						...
						for (ListIterator iter2 = alist.listIterator(); iter2
							...
							TclList.append(interp, import_list,
									TclString.newInstance((String) classTable
											.get(elem2)));
						}
		...
		for (i = startIdx; i < objv.length; i++) {
			...
			if (elem.equals("int") || elem.equals("boolean")
					|| elem.equals("long") || elem.equals("float")
					|| elem.equals("double") || elem.equals("byte")
					|| elem.equals("short") || elem.equals("char")) {
				throw new TclException(interp, "cannot " + operation
						+ " primitive type \"" + elem + "\"");
			}
			...
			if (!forget) {
				...
				if (inGlobal) {
					...
					throw new TclException(interp, "cannot import \""
							+ fullyqualified
				...
				if (c == null) {
					...
					Class inner = null;
					try {
						inner = JavaInvoke.getClassByName(interp,
								fullyqualified);
					...
					if (inner != null && JavaInvoke.isInnerClass(inner)) {
						...
						throw new TclException(interp, "cannot import class \""
								+ fullyqualified + "\", it does not exist");
					}
				}
			}
			...
			if (forget) {
				if (classTable.get(class_name) == null) {
					throw new TclException(interp, "cannot forget class \""
							+ fullyqualified + "\", it was never imported");
				}
			}
		...
		if (forgetClasses.size() != 0) {
			if (debug) {
				...
				for (ListIterator iter = forgetClasses.listIterator(); iter
						.hasNext();) {
			...
			for (ListIterator iter = forgetClasses.listIterator(); iter
				...
				if (ind == -1) {
					throw new TclRuntimeError(
							"unexpected : no package in forget class");
				}
				...
				ArrayList class_list = (ArrayList) packageTable
						.get(class_package);
				...
				if (class_list.remove(cindex) == null) {
					throw new TclRuntimeError(
							"could not remove element at index " + cindex
									+ " from {" + class_list
				...
				if (class_list.size() == 0) {
					if (packageTable.remove(class_package) == null) {
						throw new TclRuntimeError("could not remove "
								+ class_package + " from packageTable");
					}
				}
		...
		if (importClasses.size() != 0) {
			if (debug) {
				...
				for (ListIterator iter = importClasses.listIterator(); iter
						.hasNext();) {
					System.out.print((String) iter.next());
					System.out.print(" ");
				}
			...
			for (ListIterator iter = importClasses.listIterator(); iter
				...
				if (ind == -1) {
					throw new TclRuntimeError(
							"unexpected : no package in import class");
				}
	...
	/*
	 * public static void debugPrint(Interp interp) { HashMap classTable =
	 * interp.importTable[0]; HashMap packageTable = interp.importTable[1];
	 * 
	 * for (Iterator iter = classTable.entrySet().iterator(); iter.hasNext() ;)
	 * { Map.Entry entry = (Map.Entry) iter.next(); String elem = (String)
	 * entry.getKey(); String value = (String) entry.getValue(); String value2 =
	 * (String) classTable.get(elem); if (value != value2) { throw new
	 * TclRuntimeError("unexpected mismatch: \"" + value + "\" != \"" + value2 +
	 * "\""); }
	 * 
	 ...
	 * ;) { Map.Entry entry = (Map.Entry) iter.next(); String elem = (String)
	 * entry.getKey(); ArrayList alist = (ArrayList) entry.getValue(); ArrayList
	 * alist2 = (ArrayList) packageTable.get(elem); if (alist != alist2) { throw
	 * new TclRuntimeError("unexpected mismatch: \"" + alist + "\" != \"" +
	 * alist2 + "\""); }
	 * 
	 ...
	 * 
	 * Results: Returns the fully qualified name if it was imported. If the
	...
	public static String getImport(Interp interp, // Current interpreter.
